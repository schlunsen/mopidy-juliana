...
class JulianaSystemError(Exception):
    pass


class JulianaSystemPermissionError(JulianaSystemError):
...
class JulianaSystemThread(Thread):
    _USE_SUDO = True
    ...
    def run(self):
        ...
        try:
            self.can_run()
        except JulianaSystemError as e:
            logger.error(e)
    ...
    def can_run(self, *args, **kwargs):
        ...
        if exitCode > 0:
            raise JulianaSystemPermissionError(self.script_path)
        else:
            return True
