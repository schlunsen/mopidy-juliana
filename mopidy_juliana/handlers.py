...
class WebsocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def initialize(self, core, config):
        ...
        juliana.ioloop = self.ioloop  # Make available elsewhere in the Frontend
    ...
    def open(self):
        ...
        # connections
        client = {
            "connection_id": juliana.generateGuid(),
            "ip": ip,
        ...
        juliana.add_connection(connection=self, client=client)

    async def on_message(self, message):
        logger.debug("juliana websocket message received: " + message)
        ...
        if "method" in message:
            ...
            if hasattr(juliana, message["method"]):
                try:

                    # For async methods we need to await, but it must be
                    # ommited for syncronous methods
                    if asyncio.iscoroutinefunction(
                        getattr(juliana, message["method"])
                    ):
                        await getattr(juliana, message["method"])(
                            ioloop=self.ioloop,
                            data=params,
    ...
    def on_close(self):
        juliana.remove_connection(connection_id=self.connection_id)
    ...
    def handle_result(self, *args, **kwargs):
        ...
        juliana.send_message(data=data)
...
class HttpHandler(tornado.web.RequestHandler):
    ...
    async def get(self, slug=None):
        ...
        # make sure the method exists
        if hasattr(juliana, slug):
            try:

                # For async methods we need to await, but it must be ommited
                # for syncronous methods
                if asyncio.iscoroutinefunction(getattr(juliana, slug)):
                    await getattr(juliana, slug)(
                        ioloop=self.ioloop,
                        request=self,
                        callback=lambda response, error=False: self.handle_result(
